...
def drawStartScreen():  # 开始界面
    ...
    drawText('Pressed  any  key  to  start', font, TEXTCOLOR, screen, 300, 170)
    pygame.display.update()
    # waitForPlayerToPressKey()
def drawWinScreen(result):  # 比赛结束，显示结果界面
    if result == BLUEWIN:
        loadImage("BLUEWIN.png", (50, 150), WINSIZE)
    if result == REDWIN:
        loadImage("REDWIN.png", (50, 150), WINSIZE)
    # waitForPlayerToPressKey()
    pygame.display.update()
...
def drawPawnProAndValue():
    pro = getPawnPro(S)
    value = getLocValue(S)
    value = getPawnValue(pro, value)
    drawText('Value', font, BLACK, screen, 7, 0.2)
    drawText('Pro', font, BLACK, screen, 8, 0.3)
    drawText('Value', font, BLACK, screen, 9, 0.2)
    drawText('Pro', font, BLACK, screen, 10, 0.3)
    for p in range(1, 13):
        if p < 7:
            drawPawn(p, 6, 2*(p-1)+0.3, (30, 30))
            drawPawn(p, 7, 2*(p-1)+0.3, (30, 30))
            drawText(str(round(value[p-1], 2)),
                     font, BLACK, screen, 7, 2*p+0.1)
            drawText(str(round(pro[p-1], 2)), font, BLACK, screen, 8, 2*p+0.1)
        else:
            drawPawn(p, 8, 2*(p-7)+0.3, (30, 30))
            drawPawn(p, 9, 2*(p-7)+0.3, (30, 30))
            drawText(str(round(value[p-1], 2)), font,
                     BLACK, screen, 9, 2*(p-6)+0.1)
            drawText(str(round(pro[p-1], 2)), font,
                     BLACK, screen, 10, 2*(p-6)+0.1)
...
if __name__ == '__main__':
    '''
    可选测试对象
    Red  ：BetaCat1.0 | BetaCat2 | Human | GoAhead |
